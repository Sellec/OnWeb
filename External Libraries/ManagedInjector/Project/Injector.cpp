...
static unsigned int WM_LAUNCH = ::RegisterWindowMessage(L"Injector_LaunchMethod!");
static unsigned int WM_LOADASSEMBLY = ::RegisterWindowMessage(L"Injector_LoadAssembly!");
			...
			if (hProcess)
			{
				...
				if (acmRemote)
				{
					...
					if (_messageHookHandle)
					{
						...
						::SendMessage((HWND)windowHandle.ToPointer(), WM_LAUNCH, (WPARAM)acmRemote, 0);
...
void Injector::LoadAssembly(System::IntPtr windowHandle, System::String^ assembly)
{
	System::String^ assemblyClassAndMethod = assembly + "$";
	pin_ptr<const wchar_t> acmLocal = PtrToStringChars(assemblyClassAndMethod);
		...
		if (processID)
		{
			...
			if (hProcess)
			{
				...
				if (acmRemote)
				{
					...
					{
						LogMessage("SetWindowsHookEx successful", true);
						::SendMessage((HWND)windowHandle.ToPointer(), WM_LOADASSEMBLY, (WPARAM)acmRemote, 0);
						::UnhookWindowsHookEx(_messageHookHandle);
					}
...
LRESULT __stdcall MessageHookProc(int nCode, WPARAM wparam, LPARAM lparam)
{
	if (nCode == HC_ACTION)
	{
		...
		if (msg != NULL && msg->message == WM_LAUNCH)
		{
			try
			{
				System::Diagnostics::Debug::WriteLine("Got WM_LAUNCH message");
			...
			{
				System::Diagnostics::Debug::WriteLine(String::Format("Error: ", ex->Message));
			}
		}
		else if (msg != NULL && msg->message == WM_LOADASSEMBLY)
		{
			try
			{
				System::Diagnostics::Debug::WriteLine("Got WM_LOADASSEMBLY message");
				...
				if (assembly != nullptr)
				{
					System::Diagnostics::Debug::WriteLine(String::Format("Just loaded assembly '{0}' as '{1}'", acmSplit[0], assembly->FullName));
				}
			}
			catch (Exception^ ex)
			{
