...
(function($){
	...
	var SEARCH_CODE = '<span class="' + CLASS_JQUERY_TREE_SEARCH + '">Поиск: <input type="text" size="10"></span>';
	...
	$.fn.extend({
		...
		Tree: function(){
			...
			$(this)
				...
				.parent('.' + CLASS_JQUERY_TREE_CONTROLS).find('.' + CLASS_JQUERY_TREE_SEARCH + ' input[type=text]')
					.bind('keyup', function(){
						//console.log($(this).val(), $(this).val().length);
						
						var th = $(this).closest('.' + CLASS_JQUERY_TREE_CONTROLS).parent().find('.' + CLASS_JQUERY_TREE);
						var search = $(this).val();
						
						if (search.length == 0) $("li", th).show();//find('a').css('color', 'green');//.show();
						else
						{
							$("li", th).hide();//find('a').css('color', 'red');//();
							//console.log("searching...");
							$("li", th).each(function(){
								if ($(this).children('label').text().toLowerCase().indexOf(search.toLowerCase()) > -1) 
								{
									//console.log($(this).children('label').text());
								   
									$(this).show();//find('a:first').css('color', 'blue');
									$(this).find('li').each(function(){
										$(this).show();//.find('a:first').css('color', 'yellow');
									});
									$(this).parentsUntil(th, 'li').each(function(){
										$(this).show();//find('a:first').css('color', 'brown');
									});
								}
							});
			...
			$('label > input:checkbox', this).click(function(){
				setLabelClass(this);
				checkCheckbox(this);
			}).change(function(){
				setLabelClass(this);
	...
	function checkParentCheckboxes(checkboxElement){
		...
		var allCheckboxes = closestNode.find('label > input:checkbox');
		var checkedCheckboxes = closestNode.find('label > input:checkbox:checked');
	...
	function checkCheckbox(checkboxElement){
		...
		$(checkboxElement).closest('li').find('label > input:checkbox').each(function(){
