...
<{block 'body'}>
    ...
    <ul id="goods_category">
     <{foreach from=$ad.sub item=ad2 key=id2}><li><p><a href="/@Module.Name/cat/<{$id2}>" title="<{$ad2.name|re_quote}>" class="a_title"><{$ad2.name}></a></p><{/foreach}>
    ...
    <li<{if $nomar==0}> class="nomargin"<{/if}>>
     <div class="ititle"><a href="/@Module.Name/item/<{$items[$id2]}>" title="<{$items[$id2].name|re_quote}>"><{$items[$id2].name}></a></div>
     <div class="item_photo" id="ip_<{$id2}>">
      <a href="/@Module.Name/item/<{$id2}>" title="<{$items[$id2].name|re_quote}>" style="background-image:url(/data/<{if isset($items[$id2].photo[0].preview_file)}>photo/<{$items[$id2].photo[0].preview_file}><{else}>img/nophoto.jpg<{/if}>)"></a>
     </div>
     ...
     <div class="item_info">
      ...
      <div class="addcart">
       <form action='/@Module.Name/cart_add/<{$id2}>' method='post' id='add_form_<{$id2}>' class="addcartid">
