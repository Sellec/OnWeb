...
$(function() {
   ...
   <{if $data_cats|@count>0}>
    ...
    <{foreach from=$data_cats item=ad key=id}>
        <li><{*<p><a href="/@Module.Name/cat/<{$id}>" title="<{$ad.name|re_quote}>" class="a_title"><{$ad.name}></a>*}></p>
        <a href="/@Module.Name/cat/<{$id}>" title="<{$ad.name|re_quote}>" class="a_img" style="background-image:url(/<{if isset($ad.image) && $ad.image|strlen>0}><{$ad.image}><{else}>data/img/nophoto.jpg<{/if}>);"></a>
        <div class="a_text"><{$ad.description}><a href="/@Module.Name/cat/<{$id}>" title="<{$ad.name|re_quote}>" class="a_text_link"></a></div>
