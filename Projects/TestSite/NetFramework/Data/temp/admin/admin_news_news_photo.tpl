...
function photo_delete(tr_node)
{
    try {  
        ...
        if ( confirm('Вы действительно хотите удалить фотографию '+mPhotosArray[data[1]]['main_file']+'"?') )
        {
            ...
            aj.load("/admin/madmin/@Module.UrlName/news_photo_delete/<{$data.id}>/"+mPhotosArray[data[1]]['main_file'],'photo_result');
        }
    } catch(err) { alert(err); }
}
...
$(document).ready(function(){
    ...
    try {
        ...
        changeTitle('Управление фотографиями новости');
    getResultAnim($("#form_add_photo"),$("#add_photo_result"));
    } catch (err) { alert(err); }
    
});
</script>
<h2>Фотографии новости №<{$data.id}></h2>
<div id='photo_result'></div>
 ...
 <tr>
  <td class="center" colspan='10'>
   <form action='/admin/madmin/@Module.UrlName/news_photo_new/<{$data.id}>' method='post' id='form_add_photo'>
   ...
   <p style="float:left"><a href="/admin/madmin/@Module.UrlName/news_photo/<{$data.id}>" title="">Обновить список фотографий</a></p>
  </td>
 </tr>
</table>
