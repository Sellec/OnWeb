...
function photo_delete(tr_node)
{
    try {  
        ...
        if ( confirm('Вы действительно хотите удалить фотографию '+mPhotosArray[data[1]]['main_file']+'"?') )
        {
            ...
            aj.load("/admin/madmin/@Module.Name/item_photo_delete/<{$data.id}>/"+mPhotosArray[data[1]]['main_file'],'photo_result');
        }
    } catch(err) { alert(err); }
}
...
$(document).ready(function(){
    ...
    try {
        ...
        aj = new ajaxRequest();
        aj.userOnLoad = function(){endAnim($("#add_photo_result"))};
        aj.load_form('form_add_photo',null,'add_photo_result');
        stAnim();
        
        changeTitle('Управление фотографиями баннеров');
    } catch (err) { alert(err); }
});
</script>
<h2>Фотографии баннера №<{$data.id}></h2>
<div id='photo_result'></div>
 ...
 <tr>
  <td class="center" colspan='10'>
   <form action='/admin/madmin/@Module.Name/item_photo_new/<{$data.id}>' method='post' id='form_add_photo'>
   ...
   <p style="float:left"><a href="/admin/mnadmin/@Module.Name/item_photo/<{$data.id}>" title="">Обновить список фотографий</a></p>
  </td>
 </tr>
</table>
