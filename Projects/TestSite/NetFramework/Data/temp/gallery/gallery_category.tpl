...
<{block 'body'}>
  <div class="subpath"><a href="/" title="">Главная</a> &rarr; <a href="/@Module.Name" title="">Фотогалерея</a></div>
  <h1><{$data.name}></h1>
  ...
  <{foreach from=$data_cats item=ad key=id}>
   <li><a href='/@Module.Name/cat/<{$ad.id}>' title="<{$ad.name|re_quote}>"><{$ad.name}></a></li>
  <{/foreach}>
  </ul>
  <{/if}>
  ...
  <ul id="gal_gals">
   ...
   <li<{if $nomar==0}> style="margin-right:0;"<{/if}>><a href='/@Module.Name/gallery/<{$ad.id}>' class='category_view'><img src="/data/photo/<{$ad.photo.preview_file}>" alt="<{$ad.name|re_quote}>" /></a>
