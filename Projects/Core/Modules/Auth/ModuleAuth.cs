...
namespace OnWeb.Modules.Auth
{
    ...
    public abstract class ModuleAuth : ModuleCore<ModuleAuth>, IUnitOfWorkAccessor<CoreContext>
    {
        ...
        /// <summary>
        /// Запоминает адрес <paramref name="requestedAddress"/>, запрошенный пользователем, ассоциированным с текущим активным контекстом (см. <see cref="UserContextManager{TAppCoreSelfReference}.GetCurrentUserContext"/>).
        /// </summary>
        ...
        /// <summary>
        /// Возвращает адрес, запомненный модулем во время последнего вызова <see cref="RememberUserContextRequestedAddressWhenRedirectedToAuthorization(IUserContext, Uri)"/> для пользователя, ассоциированного с текущим активным контекстом (см. <see cref="UserContextManager{TAppCoreSelfReference}.GetCurrentUserContext"/>).
        /// </summary>
