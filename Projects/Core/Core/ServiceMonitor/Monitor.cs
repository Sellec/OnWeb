...
namespace OnWeb.Core.ServiceMonitor
{
    ...
    public class Monitor : CoreComponentBase<ApplicationCore>, IComponentSingleton<ApplicationCore>, IUnitOfWorkAccessor<UnitOfWork<DB.Journal>>
    {
        ...
        /// </summary>
        /// <param name="service">Сервис, для которого производится регистрация состояния.</param>
        /// <param name="eventType">См. <see cref="Journaling.IManager.RegisterEvent"/>.</param>
        /// <param name="eventInfo">См. <see cref="Journaling.IManager.RegisterEvent"/>.</param>
        /// <param name="eventInfoDetailed">См. <see cref="Journaling.IManager.RegisterEvent"/>.</param>
        /// <param name="exception">См. <see cref="Journaling.IManager.RegisterEvent"/>.</param>
        public void RegisterServiceEvent(IMonitoredService service, Journaling.EventType eventType, string eventInfo, string eventInfoDetailed = null, Exception exception = null)
        {
            ...
            if (serviceJournal != null) AppCore.Get<Journaling.IManager>().RegisterEvent(serviceJournal.IdJournal, eventType, eventInfo, eventInfoDetailed, null, exception);
        }
        ...
        private DB.JournalName GetJournalName(IMonitoredService service)
        {
            ...
            {
                var result = AppCore.Get<Journaling.IManager>().RegisterJournal(1, service.ServiceName, "ServiceMonitor_" + key.ToString());
